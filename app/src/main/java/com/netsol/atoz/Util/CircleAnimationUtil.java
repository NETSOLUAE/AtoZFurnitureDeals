...
public class CircleAnimationUtil {
    ...
    private OnAnimationFails callback;

    public CircleAnimationUtil() {
    }

    public void setOnAnimationFailListener(OnAnimationFails callback) {
        this.callback = callback;
    }

    public interface OnAnimationFails {
    ...
    public CircleAnimationUtil setTargetView(Drawable view) {
        mTarget = view;
        setOriginRect(mTarget.getBounds().width(), mTarget.getBounds().height());
        return this;
    }
    ...
    public CircleAnimationUtil setDestView(Drawable view) {
        mDest = view;
        setDestRect(mDest.getBounds().height(), mDest.getBounds().width());
        return this;
    }
    ...
    private boolean prepare() {
        if (mContextReference.get() != null) {
            ...
            mBitmap = drawViewToBitmap(mTarget, mTarget.getBounds().width(), mTarget.getBounds().height());
            if (mBitmap != null) {
                if (mImageView == null)
...
//            mTarget.getLocationOnScreen(src);
                ...
                FrameLayout.LayoutParams params = new FrameLayout.LayoutParams(mTarget.getBounds().width(), mTarget.getBounds().height());
                params.setMargins(0, ((int) destY) + 40, 0, 0);
...
//                Animator translatorX = ObjectAnimator.ofFloat(mImageView, View.X, x, x + dest[0] - (src[0] + (originX * scaleFactor - 2 * endRadius * scaleFactor) / 2) + (0.5f * destX - scaleFactor * endRadius));
                Animator translatorX = ObjectAnimator.ofFloat(mImageView, View.X, x, x * 40);
...
//                Animator translatorY = ObjectAnimator.ofFloat(mImageView, View.Y, y, y + dest[1] - (src[1] + (originY * scaleFactor - 2 * endRadius * scaleFactor) / 2) + (0.5f * destY - scaleFactor * endRadius));
                Animator translatorY = ObjectAnimator.ofFloat(mImageView, View.Y, y, y / 80);
                ...
                Animator disappearAnimatorY = ObjectAnimator.ofFloat(mImageView, View.SCALE_Y, scaleFactor/5, 0);
                Animator disappearAnimatorX = ObjectAnimator.ofFloat(mImageView, View.SCALE_X, scaleFactor/5, 0);
    ...
    private Bitmap drawViewToBitmap(Drawable view, int width, int height) {
